...
ALGraph* makeALGraph(ALGraph& G, char str[100], int num) {
    ...
    // 输入课程的先修课程号
    printf("请输入课程的先修课程号:(三位字符，课程间用空格隔开)\n");
    ...
    while (temp != NULL) {
        if (temp->result != NULL) {
            ...
            while (i < num && temp->result[i] != NULL) {  // 避免越界并确保不超过 num
                ...
                int v = search(G.vexs, temp->result[i + 1], G.n);
                AdjVexNode* r = NULL; int e=0;
                for (r = G.vexs[v].firstArc; r != NULL; r = r->next)
                {
                    if (strncpy_s(r->Adjvex, temp->result[i], 3))
                    {
                        return e = 1;
                    }
                    return 0;
...
int main() {
    ...
    LinkList L = NULL;
    FILE* file_ptr;
    errno_t err = fopen_s(&file_ptr, "course_sort.txt", "a");
    if (err != 0)
    {
        printf("\n打开文件出错\n");
        return 0;
    ...
    printf("请输入学期总数和每学期学分上限（两个数字间用空格隔开）\n");
    char str[100];
    int total, max, num;
    scanf_s("%d %d", &total, &max);
    if (total>12||total<1||max<1)
    {
        printf("数据不合理,程序终止");
        return 0;
    }
    ...
    printf("请输入所有课程号(课程号为三位字符，课程间用空格隔开)\n");
    getchar();
    ...
    while (c != 'D') {
        ...
        {
            printf("以上结果均已被储存至course_sort.txt\n");
            fclose(file_ptr);
            break;
            return 0;
        }
